...
def sh(cmd: list, env=None, cwd=None, check=True):
    ...
    try:
        ...
        console.print(f"[red]✗[/red] Command failed: {' '.join(cmd)}" if RICH_AVAILABLE else f"✗ Command failed: {' '.join(cmd)}")
        console.print(f"[red]Error:[/red] {e.stderr}" if RICH_AVAILABLE else f"Error: {e.stderr}")
        raise
    except FileNotFoundError:
        console.print(f"[red]✗[/red] Command not found: {cmd[0]}" if RICH_AVAILABLE else f"✗ Command not found: {cmd[0]}")
        raise
...
def get_python_executable(python_version: str = None) -> str:
    ...
    if python_version:
        if platform.system() == "Windows":
            ...
            try:
                result = sh([f"py", f"-{python_version}", "--version"], check=True)
...
def check_existing_venv(venv_path: Path) -> bool:
    ...
    if venv_path.exists():
        print_warning(f"Virtual environment already exists: {normalize_path(venv_path)}")
        return True
...
def create_venv(venv_path: Path, python_cmd: str, recreate: bool = False):
    ...
    if venv_path.exists() and recreate:
        print_info(f"Removing existing virtual environment: {normalize_path(venv_path)}")
    ...
    if not venv_path.exists():
        ...
        if python_cmd.startswith("py -"):
            ...
            sh([f"py", f"-{version}", "-m", "venv", str(venv_path)])
        else:
...
def upgrade_pip(venv_python: str, use_uv: bool = False):
    ...
    if use_uv:
        ...
        try:
            sh([venv_python, "-m", "pip", "install", "--upgrade", "pip", "setuptools", "wheel"])
            print_success("Upgraded pip, setuptools, and wheel")
        except subprocess.CalledProcessError:
            print_warning("Pip upgrade failed (network timeout), continuing with existing pip version")
...
def install_project_manual(venv_path: Path, extras: str = "dev"):
    ...
    python_dirs = list((venv_path / "lib").glob("python*")) if (venv_path / "lib").exists() else []
...
def install_project(venv_path: Path, extras: str = "dev", editable: bool = True, use_uv: bool = False):
...
def install_precommit_hooks(venv_path: Path, skip_precommit: bool = False):
    ...
    try:
        ...
        print_warning("Failed to install pre-commit hooks. Run manually: pre-commit install")
...
def print_activation_instructions(venv_path: Path):
    ...
    if RICH_AVAILABLE:
        ...
        activation_text.append("Virtual environment created successfully!\n\n", style="bold green")
...
def write_bootstrap_log(venv_path: Path, start_time: float, success: bool):
    ...
    if success:
        try:
            ...
            # Get package version
            result = sh([venv_python, "-m", "pip", "show", "transform-myd-minimal"], check=True)
            for line in result.stdout.split('\n'):
                if line.startswith('Version:'):
                    log_content.append(f"Package version: {line.split(':', 1)[1].strip()}")
                    break
        except subprocess.CalledProcessError:
    ...
    print_info(f"Bootstrap log written to: .bootstrap.log")
...
Examples:
  ...
  python dev_bootstrap.py --no-precommit     # Skip pre-commit setup
    ...
    parser.add_argument(
        "--extras",
        default="dev",
        help="Package extras to install (default: dev)"
    )
    ...
    parser.add_argument(
        "--uv",
        action="store_true",
        help="Use uv for package installation if available"
    )
