...
def test_no_html_flag():
    ...
    result = run_command(["index_source", "--object", "test", "--variant", "test", "--no-html"])
    # Command will fail due to missing files, but should not fail due to unknown flag
    assert "--no-html" not in result.stderr or "unrecognized arguments" not in result.stderr


def test_html_dir_flag():
    """Test that --html-dir flag is accepted."""
    with tempfile.TemporaryDirectory() as tmpdir:
        result = run_command(["index_source", "--object", "test", "--variant", "test", "--html-dir", tmpdir])
        # Command will fail due to missing files, but should not fail due to unknown flag
        assert "--html-dir" not in result.stderr or "unrecognized arguments" not in result.stderr
