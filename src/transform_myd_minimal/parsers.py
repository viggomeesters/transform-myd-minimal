...
def read_excel_headers(path: Path, sheet: str = "Sheet1", header_row: int = 1, 
                      ignore_data_below: bool = True) -> List[str]:
    """
    ...
    try:
        ...
        df = pd.read_excel(path, sheet_name=sheet, header=header_row - 1, nrows=0 if ignore_data_below else None)
...
class SpreadsheetMLParser:
    ...
    def find_header_row(self, worksheet: ET.Element, expected_headers: List[str]) -> Optional[int]:
        """
        ...
        for row_idx, row in enumerate(rows):
            ...
            if len(cells) >= len(expected_headers):
                ...
                exact_matches = sum(1 for expected in expected_lower 
                                  if any(expected == actual for actual in row_headers))
                
                # Count partial matches
                partial_matches = sum(1 for expected in expected_lower 
                                    if any(expected in actual or actual in expected 
                                          for actual in row_headers if actual))
                
                # If we have most exact matches or good partial matches, this is our header row
                if exact_matches >= len(expected_headers) * 0.8 or partial_matches >= len(expected_headers) * 0.7:
                    return row_idx
    ...
    def parse_target_fields(self, worksheet_name: str = "Field List", 
                          header_config: Optional[Dict[str, str]] = None) -> List[Dict[str, Any]]:
        """
        ...
        if header_row_idx is None:
            raise ValueError(f"Header row not found with expected headers: {expected_headers}")
...
def parse_source_and_targets(source_xlsx_path: Path, target_xml_path: Path,
                           source_config: Optional[Dict[str, Any]] = None,
                           target_config: Optional[Dict[str, Any]] = None) -> Tuple[List[str], List[Dict[str, Any]]]:
    """
    ...
    target_fields = xml_parser.parse_target_fields(
        target_config['worksheet_name'],
        target_config['header_match']
    )
