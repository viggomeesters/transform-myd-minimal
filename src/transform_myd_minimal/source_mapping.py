...
class SourceBasedMatcher:
    ...
    def match_sources_to_targets(self, source_headers: List[str], target_fields: List[Dict[str, Any]], 
                                priority_fields: List[str] = None) -> Dict[str, Any]:
        """
        ...
        for source_header in source_headers:
            ...
            if not best_match and self.fuzzy_config.enabled:
                ...
                for target in target_fields:
                    ...
                    if self.synonym_matcher.is_synonym_match(source_header, target.get('sap_field', '')):
                    ...
                    for priority_field in priority_fields:
                        ...
                        lev_sim = self.fuzzy_matcher.levenshtein_similarity(source_norm, target_norm) if self.fuzzy_config.use_levenshtein else 0.0
                        jw_sim = self.fuzzy_matcher.jaro_winkler_similarity(source_norm, target_norm) if self.fuzzy_config.use_jaro_winkler else 0.0
                        
                        combined_sim = (lev_sim * self.fuzzy_config.levenshtein_weight + 
                                      jw_sim * self.fuzzy_config.jaro_winkler_weight)
                        
                        if combined_sim >= self.fuzzy_config.threshold and combined_sim > best_confidence:
                            best_match = target
        ...
        unmatched_targets = [target for target in target_fields 
        ...
        return {
            ...
            'stats': {
                ...
                'coverage_percentage': (len(matches) / len(source_headers) * 100) if source_headers else 0
            }
        }


def generate_targets_yaml(target_fields: List[Dict[str, Any]], output_path: Path) -> None:
    """
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        f.write(f"# Target fields metadata generated by transform-myd-minimal\n")
...
def run_source_based_mapping(config, args=None) -> None:
    ...
    try:
        source_headers, target_fields = parse_source_and_targets(
            ...
            target_config={'worksheet_name': config.target_xml['worksheet_name'],
    ...
    mapping_result = matcher.match_sources_to_targets(
        ...
        config.matching['target_label_priority']
    )
