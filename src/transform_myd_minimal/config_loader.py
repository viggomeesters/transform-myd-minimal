...
class Config:
    ...
    def _load_from_file(self, config_path: Path) -> None:
        ...
        try:
            ...
            if config_data:
                ...
                if mapping_config:
                    self.mapping_from_sources = mapping_config.get(
                        "from_sources", self.mapping_from_sources
                    )
                    ...
                    if "target_xml" in mapping_config:
                        ...
                        for key, value in target_config.items():
                            if (
                                key in self.target_xml
                                and isinstance(self.target_xml[key], dict)
    ...
    def merge_with_cli_args(self, args) -> None:
        ...
        if (
            hasattr(args, "source_headers_xlsx")
            and args.source_headers_xlsx is not None
        ):
            self.source_headers["path"] = args.source_headers_xlsx
        if (
            hasattr(args, "source_headers_sheet")
            and args.source_headers_sheet is not None
        ):
        ...
        if (
            hasattr(args, "target_xml_worksheet")
            and args.target_xml_worksheet is not None
        ):
